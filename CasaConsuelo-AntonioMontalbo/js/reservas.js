...
fetch("/CasaConsuelo-AntonioMontalbo/controller/reservas_controller.php?obtener=1")
...
function calcularPrecio(){
    ...
    if(fechaEntrada != "" && fechaSalida != ""){
        ...
        let precioNoche = numPersonas * precioBase;
...
let formularioReserva = document.querySelector("form");
...
formularioReserva.addEventListener("submit", function(e){
    ...
    for(let i = 0; i < reservas.length; i++){
        let entradaReserva = new Date(reservas[i].f_entrada);
        let salidaReserva = new Date(reservas[i].f_salida);
        let entradaNueva = new Date(fechaEntrada);
        let salidaNueva = new Date(fechaSalida);

        if(entradaNueva < salidaReserva && salidaNueva > entradaReserva){
    ...
    if(precioOculto.value == "" || precioOculto.value == 0){
        alert("Debes completar la reserva");
        e.preventDefault();
        return;
    }
